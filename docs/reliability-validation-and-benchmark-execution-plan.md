...
- [ ] 所有单元测试通过。
- [ ] 所有 MySQL/Redis Testcontainers 测试通过。
- [ ] Toxiproxy 故障测试通过。
- [ ] Surefire 没有失败、错误或跳过的核心测试。
- [ ] JaCoCo 报告能够生成。
...
- [ ] 最终状态只允许 `SETTLED` 或 `CANCELLED`。
- [ ] 单个订单只能出现 SETTLE 或 CANCEL 中的一种资金路径。
- [ ] 发布者 FROZEN 余额最终归零。
- [ ] 每轮竞争结束后 5 项对账通过。

#### Redis 故障

- [ ] Redis 断开时真实抢单仍然恰好一个赢家。
- [ ] Redis 超时不会造成请求长时间阻塞。
- [ ] Redis 故障不会泄漏为 HTTP 500。
- [ ] Redis 恢复后 marker 和延迟事件能够重建。

#### Outbox

- [ ] 领取事件使用独立短事务。
- [ ] Redis 网络调用不持有 Outbox 数据库行锁。
- [ ] 状态更新校验事件租约。
- [ ] 两个 Worker 不会同时持有同一事件。
- [ ] PROCESSING 租约过期后能够重新领取。
- [ ] 重复消费保持 Redis 派生状态幂等。
- [ ] 毒事件不会阻塞同批其他事件。
- [ ] 连续失败最终进入 DEAD。
- [ ] 管理接口可以重放 DEAD 事件。
...
- [ ] 未登录写请求返回 401。
- [ ] 无效、过期或错误签名 JWT 返回 401。
- [ ] USER 访问管理接口返回 403。
- [ ] 用户不能查看他人钱包或为他人充值。
- [ ] 订单身份只能来自认证上下文。
- [ ] 注册密码使用 BCrypt，响应不暴露摘要。
- [ ] 旧的公开用户创建入口已经关闭。
...
- [ ] 应用 readiness 成功。
- [ ] Prometheus 正常抓取应用。
- [ ] Grafana 正常加载 datasource 和 dashboard。
- [ ] Flyway 在空数据库上完成所有迁移。
...
- [ ] 所有业务指标能在 Prometheus 查询到。
- [ ] Redis 故障时指标能体现降级到 DB CAS。
- [ ] 人为制造 DEAD 和账实漂移时告警触发。
- [ ] Grafana 图例、单位和时间窗口清晰。
...
- [ ] 所有原始结果以 JSON 或 CSV 保存。
- [ ] 每个结论可以追溯到场景参数和原始数据。
- [ ] Redis 开关使用相同负载进行公平对照。
- [ ] 图表由仓库中的原始数据生成。
- [ ] Redis 故障期间没有双赢家和资金异常。
- [ ] 不记录或使用无法复现的估算数字。
...
- 检查设计文档、代码和测试描述是否一致。
- 将宽泛命名的 `next-tasks.md` 内容合并或归档，避免维护两份重复清单。
...
- [ ] README 中的可靠性结论都有测试对应。
- [ ] 简历中的每个数字都能在压测报告中定位。
- [ ] 仓库无真实密码、临时数据和构建产物。
- [ ] 能在 5～10 分钟内完成一次现场故障演示。
