...
- 实现 5 项资金及状态不变量对账。
- 测试源包含 53 个 `@Test` 方法，覆盖单元、真实数据库、Redis 和 Toxiproxy 场景；空 Maven 仓库构建及完整测试已连续三次通过。
...
早期实现只设置数据库中的 `deliver_deadline_at`，未稳定写入 Redis ZSet；现已通过 Outbox 和数据库扫描兜底补齐。
...
早期送达状态更新只校验订单状态和接单人；现已在状态更新中校验 `deliver_deadline_at`，并覆盖截止前、截止后及送达/超时竞态。
...
### T3. 修正 Redis 前置过滤语义（已完成）

**历史问题**

早期 Redis 返回 `FILTERED` 后仍会读取订单，因此只能减少数据库更新，不能减少数据库总请求。
...
早期 MINT 和 PLATFORM 账户可能调用普通用户业务，平台用户作为接单人时结算分录可能落到相同唯一键并持续失败。
...
早期重复幂等键直接返回已有状态，但没有验证两次请求的业务参数是否一致。
...
以下历史 P0 事项已经由代码、测试和运行报告覆盖：

- [x] 外部化数据库和 Redis 配置，并提供 `.env.example`。
- [x] 为送达状态更新增加截止时间守卫。
- [x] 抢单成功后通过 Outbox 补写送达超时事件。
- [x] 增加 `deliver` 与 `timeoutDeliver` 并发测试。
- [x] 禁止 MINT、PLATFORM 参与普通订单流程。
- [x] 增加系统账户相关结算回归测试。
- [x] 修正 Redis 前置过滤的实现和文档描述。
- [x] 初始化 Git 并加入 `.gitignore`。

仍需在用户明确授权 Git 工作后处理：

- [ ] 审查并拆分当前工作区改动，创建提交。
- [ ] 推送远程仓库并验证远程 CI 与分支保护。

生产安全加固、数据库/Redis 高可用与灾备、支付/通知/争议等业务外围能力，以及更完整的 SLO/追踪体系，属于 `docs/outstanding-work-plan.md` 记录的后续产品化范围，不因本地验收完成而视为已交付。
